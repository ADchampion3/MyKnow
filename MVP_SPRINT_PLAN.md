...
- **首版支持**：Markdown、TXT、文本型 PDF、URL 快照；OpenAI-compatible 云端 Provider；Ollama 接口保留但不作为发布阻塞项。
- **明确不做**：多用户协作、实时编辑、原生客户端、自动网页同步、OCR 主流程、知识图谱、多 Agent、自训练模型。
...
- Markdown/TXT/文本型 PDF 上传；URL 抓取正文快照、标题、原始 URL、抓取时间。
- MIME/大小校验、内容指纹去重、版本关系、失败重试和错误日志。
- 解析、结构提取、分块、全文索引和本地向量索引；chunk 关联资料版本和定位信息。
...
4. 任一索引 chunk 能反查到资料、版本、页码/段落或 URL 定位；删除/重建向量索引不会删除主数据库记录。
5. 任务中断后点击重试可从最近安全步骤继续，最终状态和错误日志可审计。

**出口证据**：四类导入验收表、版本链查询结果、chunk 定位抽样（至少 20 个）、任务失败/重试日志。
...
- [ ] Markdown、TXT、文本型 PDF、URL 均可导入；原始资料不可变，更新保留版本关系和内容指纹。
