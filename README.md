...
The keyword path uses SQLite FTS5 plus deterministic English token/stopword handling, CJK bigrams, OR recall, title/phrase scoring, and explainable match features. Optional local mock embeddings are generated by the Worker and stored as derived rows. Set `RETRIEVAL_VECTOR_ENABLED=false` to disable them; an unavailable, timed-out, or failed provider degrades to the keyword path and records the reason in the trace. API keys are never returned in traces or logs.
