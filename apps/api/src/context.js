...
export const createApiContext = ({ config, sqlite, db, http }) => {
  ...
  const queueVersion = (resourceVersionId, requestId, reason = "requested") => {
    const timestamp = now();
    const value = { id: crypto.randomUUID(), type: "resource:process", resourceVersionId, payload: JSON.stringify({ reason }), status: "queued", progress: 0, retryLimit: 3, retryCount: 0, createdAt: timestamp, updatedAt: timestamp };
  ...
  const versionView = (row) => omit(row, ["storage_key", "source_url", "fetched_at", "idempotency_key", "request_fingerprint"]);
  const runView = (row) => omit(row, ["canonical_storage_key"]);
  const taskView = (row) => row ? omit(row, ["payload"]) : row;
