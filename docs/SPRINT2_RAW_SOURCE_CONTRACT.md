...
- Multipart local file import: `name`, `knowledgeBaseId`, and `file` for `.md`, `.txt`, or best-effort `.pdf`.
...
| `processing_run.id`, `resource_version_id`, `status` | One concrete parse/index execution. Only an `indexed` run referenced by `active_processing_run_id` supplies the active FTS rows. |
...
For the local PDF full-chain check, install the optional MarkItDown PDF extra into the project uv environment, then run `node scripts/pdf-e2e.js "D:\\path\\book.pdf"`. The script uses an isolated database/storage directory, verifies source SHA-256 round-trip, waits for `queued -> running -> succeeded`, checks the canonical artifact and child-only FTS, and writes a timestamped report under `artifacts/sprint2/`. Large documents use `RESOURCE_PARSER_TIMEOUT_MS` (default `120000`).
