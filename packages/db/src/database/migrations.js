...
CREATE TABLE wiki_page_blocks (id TEXT PRIMARY KEY, page_version_id TEXT NOT NULL REFERENCES wiki_page_versions(id), block_key TEXT NOT NULL, block_type TEXT NOT NULL, ordinal INTEGER NOT NULL CHECK(ordinal >= 0), heading_path TEXT NOT NULL, content_markdown TEXT NOT NULL, content_sha256 TEXT NOT NULL CHECK(length(content_sha256)=64), UNIQUE(page_version_id, block_key));
...
export function migrate(sqlite) {
  ...
  sqlite.transaction(() => {
    ...
    sqlite.prepare("INSERT INTO schema_meta (key,value,updated_at) VALUES ('derived_schema','sprint6-personal-derived-ready',?)").run(timestamp);
  })();
