const SCHEMA_VERSION = "sprint2-raw-v4";
...
CREATE TABLE resource_versions (id TEXT PRIMARY KEY, resource_id TEXT NOT NULL REFERENCES resources(id), content_sha256 TEXT NOT NULL CHECK(length(content_sha256)=64), storage_key TEXT NOT NULL, mime_type TEXT NOT NULL, byte_size INTEGER NOT NULL CHECK(byte_size > 0), original_filename TEXT, title TEXT, parser_name TEXT, parser_version TEXT, parse_duration_ms INTEGER, chunking_config TEXT, active_processing_run_id TEXT REFERENCES processing_runs(id) DEFERRABLE INITIALLY DEFERRED, status TEXT NOT NULL DEFAULT 'pending' CHECK(status IN ('pending','processing','indexed','failed')), error_summary TEXT, idempotency_key TEXT UNIQUE, request_fingerprint TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
