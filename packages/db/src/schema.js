...
export const tasks = sqliteTable("tasks", {
  id: text("id").primaryKey(), type: text("type").notNull(), resourceVersionId: text("resource_version_id"), processingRunId: text("processing_run_id"), payload: text("payload"), status: text("status").notNull().default("queued"), progress: integer("progress").notNull().default(0), retryLimit: integer("retry_limit").notNull().default(3), retryCount: integer("retry_count").notNull().default(0), nextAttemptAt: text("next_attempt_at"), workerId: text("worker_id"), cancelRequested: integer("cancel_requested").notNull().default(0), errorCode: text("error_code"), errorSummary: text("error_summary"), startedAt: text("started_at"), finishedAt: text("finished_at"), ...timestamps
});
...
export const agentRuns = sqliteTable("agent_runs", {
  id: text("id").primaryKey(), taskId: text("task_id").notNull(), runKind: text("run_kind").notNull(), knowledgeBaseId: text("knowledge_base_id"), spaceId: text("space_id"), scopeSnapshot: text("scope_snapshot").notNull(), promptText: text("prompt_text").notNull(), promptHash: text("prompt_hash").notNull(), promptVersion: text("prompt_version").notNull(), contractVersion: text("contract_version").notNull(), provider: text("provider").notNull(), model: text("model").notNull(), egressMode: text("egress_mode").notNull(), citationPolicy: text("citation_policy").notNull().default("required"), status: text("status").notNull().default("queued"), metrics: text("metrics").notNull().default("{}"), resultJson: text("result_json"), errorCode: text("error_code"), errorSummary: text("error_summary"), idempotencyKey: text("idempotency_key"), requestFingerprint: text("request_fingerprint"), createdAt: text("created_at").notNull(), updatedAt: text("updated_at").notNull()
});
export const agentPlanItems = sqliteTable("agent_plan_items", {
  id: text("id").primaryKey(), runId: text("run_id").notNull(), ordinal: integer("ordinal").notNull(), itemType: text("item_type").notNull(), targetPageId: text("target_page_id"), basePageVersionId: text("base_page_version_id"), proposedJson: text("proposed_json").notNull(), citationsJson: text("citations_json").notNull().default("[]"), diffJson: text("diff_json"), risk: text("risk").notNull(), evidenceStatus: text("evidence_status").notNull(), validationWarnings: text("validation_warnings").notNull().default("[]"), reviewStatus: text("review_status").notNull().default("proposed"), applicationStatus: text("application_status").notNull().default("pending"), appliedPageVersionId: text("applied_page_version_id"), rollbackPageVersionId: text("rollback_page_version_id"), decisionReason: text("decision_reason"), decidedBy: text("decided_by"), decidedAt: text("decided_at"), appliedAt: text("applied_at"), errorCode: text("error_code"), errorSummary: text("error_summary"), createdAt: text("created_at").notNull(), updatedAt: text("updated_at").notNull()
});
