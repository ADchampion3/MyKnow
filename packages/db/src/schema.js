...
export const tasks = sqliteTable("tasks", {
  id: text("id").primaryKey(), type: text("type").notNull(), payload: text("payload"), status: text("status").notNull().default("queued"), progress: integer("progress").notNull().default(0), retryLimit: integer("retry_limit").notNull().default(3), retryCount: integer("retry_count").notNull().default(0), workerId: text("worker_id"), errorSummary: text("error_summary"), startedAt: text("started_at"), finishedAt: text("finished_at"), ...timestamps
});
export const taskAttempts = sqliteTable("task_attempts", {
  id: text("id").primaryKey(), taskId: text("task_id").notNull(), attemptNumber: integer("attempt_number").notNull(), status: text("status").notNull(), workerId: text("worker_id"), startedAt: text("started_at").notNull(), finishedAt: text("finished_at"), errorSummary: text("error_summary")
});
...
export const resourceVersions = sqliteTable("resource_versions", {
  id: text("id").primaryKey(), resourceId: text("resource_id").notNull(), contentSha256: text("content_sha256").notNull(), storageKey: text("storage_key").notNull(), mimeType: text("mime_type").notNull(), byteSize: integer("byte_size").notNull(), title: text("title"), sourceUrl: text("source_url"), fetchedAt: text("fetched_at"), parserName: text("parser_name"), parserVersion: text("parser_version"), parseDurationMs: integer("parse_duration_ms"), chunkingConfig: text("chunking_config"), activeProcessingRunId: text("active_processing_run_id"), status: text("status").notNull().default("pending"), errorSummary: text("error_summary"), ...timestamps
});
export const processingRuns = sqliteTable("processing_runs", {
  id: text("id").primaryKey(), resourceVersionId: text("resource_version_id").notNull(), status: text("status").notNull().default("pending"), parserName: text("parser_name"), parserVersion: text("parser_version"), chunkerName: text("chunker_name"), chunkerVersion: text("chunker_version"), chunkingConfig: text("chunking_config"), inputSha256: text("input_sha256"), canonicalStorageKey: text("canonical_storage_key"), canonicalSha256: text("canonical_sha256"), canonicalByteSize: integer("canonical_byte_size"), blockCount: integer("block_count").notNull().default(0), parentCount: integer("parent_count").notNull().default(0), childCount: integer("child_count").notNull().default(0), outputSha256: text("output_sha256"), warningCount: integer("warning_count").notNull().default(0), errorSummary: text("error_summary"), ...timestamps
});
