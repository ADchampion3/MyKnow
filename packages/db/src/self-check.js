...
assert.equal(sqlite.prepare("SELECT value FROM schema_meta WHERE key='schema_version'").get().value, "sprint5-agent-tree-v1");
assert.equal(sqlite.prepare("SELECT value FROM schema_meta WHERE key='derived_schema'").get().value, "sprint5-agent-tree-derived-ready");
...
assert.ok(sqlite.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='wiki_page_citations'").get());
sqlite.close();
